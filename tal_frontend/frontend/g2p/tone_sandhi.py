...
class ToneSandhi():
    ...
    def _neural_sandhi(self, word: str, pos: str,
        ...
        if len(word) >= 1 and word[-1] in "吧呢啊呐噻嘛吖嗨呐哦哒滴哩哟喽啰耶喔诶":
            ...
            finals[-1] = finals[-1][:-1] + "5"
        # e.g. 走了, 看着, 去过
        elif len(word) == 1 and word in "了着过" and pos in {"ul", "uz", "ug"} and len(finals) > 0:    #########报错了
            finals[-1] = finals[-1][:-1] + "5"
    ...
    def _merge_yi(self, seg: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
        ...
        #             0] == seg[i + 1][0] and seg[i - 1][1] == "v":  #判断 一 前后两个字若相同
        #         if i - 1 < len(new_seg):
        #             new_seg[i -
    ...
    def pre_merge_for_modify(
        ...
        seg = self._merge_yi(seg)       #这里有bug, 挪一挪 多了个字
    ...
    def modified_tone(self, word: str,
                      phs: str) -> List[str]:
        """
            word: 分词
            pos: 词性
        ...
        """
        # 分词
        jieba_res = self._split_word(word)
        #phs = phs.strip('/').split('/')
        start_index = 0
        modified_phs = []
        for item in jieba_res:
            word, pos = item
            word_len = len(re.findall(r'[\u4e00-\u9fff]', word))
            ph = phs[start_index : start_index+word_len]
            sm, ym = [], []
            for pinyin in ph:
                pinyin = pinyin.split()
                if len(pinyin) == 2:
            ...
            finals = self._bu_sandhi(word, ym)
            finals = self._yi_sandhi(word, finals)
            finals = self._neural_sandhi(word, pos, finals)
            finals = self._three_sandhi(word, finals)
            combined_list = []
            for a, b in zip(sm, finals):
                #combined_list.append(a)
                #combined_list.append(b)
                combined_list.append(f'{a} {b}')

            modified_phs.extend(combined_list)        
        return modified_phs
    
if __name__ == '__main__':
    ts = ToneSandhi()
    text = '一行白鹭上青天'
    phs = ['y i4', 'x ing2', 'b ai2', 'l u4', 'sh ang4', 'q ing1', 't ian1']
